...
class FeatureInputApp(App[str | tuple[str, str] | None]): # Modified return type
    ...
    def _run_external_editor_sync(self, editor_cmd: str, current_text: str, temp_file_path: str) -> None:
        ...
        try:
            ...
            # For debugging, show the structure of the command and wait mechanism.
            # The actual unique channel name will be generated by tmux_utils.
            self.call_from_thread(
                self.notify,
                (f"Editor command: {full_editor_command_for_tmux}\n"
                 f"Mechanism: tmux new-window with '{full_editor_command_for_tmux}; tmux wait-for -S <unique_channel>',\n"
                 f"then main process runs 'tmux wait-for <unique_channel>'."),
